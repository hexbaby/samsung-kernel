...
int JackClientSocket::Connect(const char* dir, const char* name, int which) // A revoir : utilisation de "which"
{
    ...
    jack_info("JackClientSocket::Connect : addr.sun_path %s, fSocket %d", addr.sun_path, fSocket);
...
int JackClientSocket::Close()
{
    ...
    if (fSocket >= 0) {
        jack_info("JackClientSocket::Close fSocket %d", fSocket);
...
int JackClientSocket::Read(void* data, int len)
{
    ...
    if(fSocket < 0){
        jack_error("JackClientSocket::Read invalid fSocket");
        return -1;
    }
    if ((res = read(fSocket, data, len)) != len) {
        if (errno == EWOULDBLOCK || errno == EAGAIN) {
            jack_error("JackClientSocket::Read time out, fd = %ld, err '%s'", fSocket, strerror(errno));
...
#endif
    if(fSocket < 0){
        jack_error("JackClientSocket::Write invalid fSocket");
        return -1;
    }
    if ((res = write(fSocket, data, len)) != len) {
        if (errno == EWOULDBLOCK || errno == EAGAIN) {
            jack_error("JackClientSocket::Write time out, fd = %ld, err '%s'", fSocket, strerror(errno));
...
int JackServerSocket::Bind(const char* dir, const char* name, int which) // A revoir : utilisation de "which"
{
    ...
    addr.sun_family = AF_UNIX;
#ifdef __ANDROID__
    memset(fName, 0 , SOCKET_MAX_NAME_SIZE);
    memset(addr.sun_path, 0, UNIX_PATH_MAX);
#endif
    BuildName(name, fName, dir, which);
    strncpy(addr.sun_path, fName, sizeof(addr.sun_path) - 1);

    jack_info("JackServerSocket::Bind : addr.sun_path %s, fSocket %d", addr.sun_path, fSocket);
...
JackClientSocket* JackServerSocket::Accept()
{
    ...
    if(fSocket < 0){
        jack_error("JackServerSocket::Accept invalid fSocket");
        return 0;        
    }
...
{
    if (fSocket >= 0) {
        jack_info("JackServerSocket::Close %s, fSocket %d", fName, fSocket);
        shutdown(fSocket, SHUT_RDWR);
        close(fSocket);
        unlink(fName);
        fSocket = -1;
        ...
        jack_log("JackServerSocket::Close");
        return -1;
    }
}
